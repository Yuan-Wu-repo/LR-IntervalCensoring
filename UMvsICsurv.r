...
hazmatrix.um <- matrix(0, 47, run)
hazmatrix.ICsurv <- matrix(0, 47, run)
...
for (run_index in 1 : run)	{
	...
	hazmatrix.um[,run_index] <- hazest	
	
	etamatrix[,run_index]<-out$par[(n.spline+1) : (n.spline+n.variable)]
	
	#processing time for the proposed uncontrained optimization 
	pt1 <- proc.time() - t0
	sumpt1 <- sumpt1 + pt1
	...
	hazmatrix.ICsurv[,run_index] <- c(fitsemi$hz)
...
haz.true <- t.seq^(1/2)
meanhaz.um <- rowMeans(hazmatrix.um)
meanhaz.ICsurv <- rowMeans(hazmatrix.ICsurv)

pdf("hazards.pdf")	
plot(c(-0.2, 5), c(-0.05, 2.5),type = "n", xlab = " ", ylab = " ")
lines(t.seq, haz.true, lty = 1, col = 1, lwd = 2)
lines(t.seq, meanhaz.um, lty = 2, col = 2, lwd = 2)
lines(t.seq, meanhaz.ICsurv, lty = 4, col = 4, lwd = 2)
legend(3, 1, c("True", "UM", "ICsurv"), lty = c(1,2,4), col = c(1,2,4))
dev.off()
